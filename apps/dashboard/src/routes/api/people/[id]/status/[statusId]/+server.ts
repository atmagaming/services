...
export const PATCH: RequestHandler = async ({ locals, request, params }) => {
  ...
  const updated = await prisma.personStatusChange.update({ where: { id: params.statusId }, data });

  invalidateCache("people");

  if (updated.status === "working") enqueueNotionSync(updated.personId, updated.status as PersonStatus);
