...
async function getAccessToken(): Promise<string> {
  const res = await fetch("https://oauth2.googleapis.com/token", {
    ...
    body: new URLSearchParams({
      client_id: apiEnv.GOOGLE_CLIENT_ID,
      client_secret: apiEnv.GOOGLE_CLIENT_SECRET,
      refresh_token: apiEnv.GOOGLE_REFRESH_TOKEN,
...
export async function uploadToDrive(filename: string, mimeType: string, buffer: Buffer): Promise<string> {
  ...
  const metadata = JSON.stringify({ name: filename, parents: [apiEnv.GOOGLE_DRIVE_DOCUMENTS_FOLDER] });
...
export async function copyDocToAgreements(fileId: string, name: string): Promise<string> {
  ...
  const res = await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}/copy`, {
    ...
    body: JSON.stringify({ name, parents: [apiEnv.GOOGLE_DRIVE_AGREEMENTS_FOLDER_ID] }),
  });
