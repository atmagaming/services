import type { InvestmentPoint, MonthlyExpense, Person, ProjectionMonth, RevenueShare, Transaction } from "@atma/types";
...
export function projectExpenses(people: Person[], endMonth: string): ProjectionMonth[] {
  ...
  for (const monthStr of months) {
    ...
    projections.push({ month: monthStr, paid: Math.round(paid), accrued: Math.round(accrued), total: Math.round(paid + accrued) });
  }
