...
import type { IdType, PersonStatus } from "@atma/types";
import { Rates, VALID_STATUSES } from "@atma/types";
import { getUser } from "../auth";
import { countMondaysInMonth, getCachedPeople, invalidateCache, isPersonActive, mapPersonRecord } from "../data";
import { apiEnv } from "../env";
import { getDriveFileStream, uploadToDrive } from "../google-drive";
import { enqueueNotionSync } from "../notion-queue";
import { createPersonNotionPage, setPersonNotionIcon, updatePersonNotion, uploadImageToNotion } from "../notion-service";
import { prisma } from "../prisma";
import { sendDocumentForSigning } from "../signer";
...
people.post("/:id/sign", async (c) => {
  ...
  if (!record) return c.json({ message: "Person not found" }, 404);

  const sorted = record.statusChanges.toSorted((a, b) => a.date.localeCompare(b.date));
  const workingChange = sorted.find((sc) => sc.status === "working");
  const inactiveChange = sorted.findLast((sc) => sc.status === "inactive");
  ...
  try {
    ...
    await prisma.signingRequest.create({
      data: { personId: c.req.param("id"), category: body.category, requestId, adminUrl, personUrl },
    });
    ...
    const url = `https://api.telegram.org/bot${apiEnv.TELEGRAM_BOT_TOKEN}/sendMessage`;
    await fetch(url, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        chat_id: apiEnv.TELEGRAM_ADMIN_USER_ID,
        text:
