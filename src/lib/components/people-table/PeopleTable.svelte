  ...
  <TableBody>
    {#each people as person (person.id)}
      ...
      <TableRow class="group cursor-pointer" onclick={() => onEditPerson(person)}>
        ...
        <TableCell class="px-4 py-2 text-sm">${person.hourlyRate.paid}/hr</TableCell>
        <TableCell class="px-4 py-2 text-sm">${person.hourlyRate.accrued}/hr</TableCell>
        <TableCell class="px-4 py-2 text-sm text-muted-foreground">{person.email || "—"}</TableCell>
        <TableCell class="px-4 py-2 text-sm text-muted-foreground">{person.telegram || "—"}</TableCell>
